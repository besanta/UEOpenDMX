...
//
//int FArtnetWorker::dmx_handler(artnet_node n, int port, void* d)
//{
//	//UE_LOG(LogArtnet, Log, TEXT("DMX Handle %d"), port);
//	FArtnetWorker* worker = (FArtnetWorker*)d;
//	int length;
//	if (worker) {
//		uint8* data = (uint8*)artnet_read_dmx(n, port, &length);
//		FMemory::Memcpy(worker->Buffer[port].GetData() + 1, data, length);
...
//}
//
//int FArtnetWorker::program_handler(artnet_node n, void* d)
//{
//	UE_LOG(LogArtnet, Log, TEXT("Program Handle"));
//	return 0;
//}
//
//int FArtnetWorker::firmware_handler(artnet_node n, int ubea, uint16_t* data, int length, void* d)
//{
...
/*
FArtnetWorker* FArtnetWorker::Listen(UArtnetNodeComponent* ArtnetComponent)
//{
	//if (!Runnable && FPlatformProcess::SupportsMultithreading()) {
	//	Runnable = new FArtnetWorker();
	//}

	//if (!Runnable->Nodes.Find(ArtnetComponent->Ip)) {
	//	artnet_node node = artnet_new(TCHAR_TO_ANSI(*ArtnetComponent->Ip), true);

	//	if (node) {
	//		TArray<TWeakObjectPtr<UArtnetNodeComponent>> listeners;
	//		listeners.Add(ArtnetComponent);
	//		Runnable->Listeners.Emplace(ArtnetComponent->Ip, listeners);

	//		Runnable->Nodes.Emplace(ArtnetComponent->Ip, node);
	//
	//		artnet_set_dmx_handler(node, &FArtnetWorker::HandleDMX, Runnable);

	//		if (artnet_start(node)) {
	//			UE_LOG(LogArtnet, Error, TEXT("Error on start %s"), *ArtnetComponent->Ip);
	//		}

	//		UE_LOG(LogArtnet, Log, TEXT("Success on start %s"), *ArtnetComponent->Ip);
	//	}

	//
	//}
	//else {
	//	Runnable->Listeners[ArtnetComponent->Ip].Add(ArtnetComponent);
	//}

	//

	//if (Runnable) {
	//	Runnable->TaskCounter.Increment();
	//}

	//return Runnable;
//}

//void FArtnetWorker::Leave(UArtnetNodeComponent* ArtnetComponent)
//{
//	if(Runnable)
//	if (auto l = Runnable->Listeners.Find(ArtnetComponent->Ip)) {
//		(*l).Remove(ArtnetComponent);
//
//		if ((*l).Num() == 0) {
//			Runnable->Listeners.Remove(ArtnetComponent->Ip);
//		}
//	}
//
//
